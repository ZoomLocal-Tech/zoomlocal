...
import ManagedContactForm from './ManagedContactForm.vue'
...
const HOW_IT_WORKS = [
  { step: '01', title: 'Grant Access', description: 'Share access to your Google Business Profile locations. One-click secure Google authorization, we never see your password.', icon: markRaw(Shield), borderFrom: 'from-green-600/30', stepText: 'text-green-500/20', iconBg: 'bg-green-500/10', iconText: 'text-green-400' },
  { step: '02', title: 'We Audit & Optimize', description: 'Our Local SEO experts run a complete profile audit, optimize your listing, and set up monitoring, protection, and content plans.', icon: markRaw(ClipboardCheck), borderFrom: 'from-emerald-600/30', stepText: 'text-emerald-500/20', iconBg: 'bg-emerald-500/10', iconText: 'text-emerald-400' },
  { step: '03', title: 'Ongoing Management', description: 'We manage reviews, publish content, track rankings, protect your profile, and send you monthly reports. You do nothing.', icon: markRaw(Zap), borderFrom: 'from-teal-600/30', stepText: 'text-teal-500/20', iconBg: 'bg-teal-500/10', iconText: 'text-teal-400' },
]
...
const MONTHLY_FEATURES = [
  "Fully managed by Fly's Local SEO experts",
  'Review monitoring & AI-powered responses',
  '24/7 profile protection & alerts',
  'Weekly GBP content & posts',
  'Keyword rank tracking',
  'Monthly performance reports',
  'Review generator (QR code + link)',
...
const WHATSAPP_URL =
  "https://wa.me/919270362196?text=Hi%2C%20I%27m%20interested%20in%20Fly%27s%20Managed%20Local%20SEO%20Services"
</script>
...
<template>
  <div class="min-h-screen bg-gray-950 text-white">
    ...
    <header class="fixed top-0 inset-x-0 z-50 bg-gray-950/80 backdrop-blur-md border-b border-gray-800/60">
      <div class="container mx-auto px-4 h-16 flex items-center justify-between">
        <a href="#" class="flex items-center gap-2">
          ...
          <span class="text-lg font-bold text-white">Fly <span class="text-gray-500 font-normal">Managed SEO</span></span>
        </a>
        <a href="#contact" class="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white text-sm font-semibold transition-colors">
          <Mail class="w-4 h-4" /> Talk to Our Team
        </a>
      </div>
    </header>
      ...
      <div class="relative z-10 container mx-auto px-4 text-center">
        <div v-reveal>
          <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-green-500/10 border border-green-500/20 mb-8">
            ...
            <span class="text-sm text-green-300">Managed by Fly's Local SEO Experts</span>
          </div>
          <h1 class="text-5xl md:text-7xl font-black mb-6 leading-tight">
            ...
            <br />
            <span class="text-white">on Google Search</span>
          </h1>
          <p class="text-xl md:text-2xl text-gray-400 max-w-3xl mx-auto mb-4 leading-relaxed">
            ...
            Fly's Local SEO experts manage your Google Business Profile so you get found first.
          </p>
          <p class="text-lg text-gray-500 max-w-2xl mx-auto mb-8">
            ...
            <span class="text-violet-400">reporting</span> — you grant access, our experts handle everything using Fly's platform.
          </p>
          ...
          <div class="relative">
            ...
            <div class="flex gap-8 items-center animate-scroll">
              <div v-for="(logo, i) in LOGO_LOOP" :key="i" class="flex-shrink-0 bg-white/90 rounded-xl px-5 py-3 flex items-center justify-center min-w-[100px]">
                <img :src="logo" alt="Client" class="h-10 w-auto object-contain" />
              </div>
            </div>
          </div>
        </div>
    ...
    <section class="py-20 relative bg-gradient-to-b from-gray-950 via-gray-900/50 to-gray-950">
      <div class="container mx-auto px-4">
        <div v-reveal class="text-center mb-12">
          ...
          <p class="text-gray-400 text-lg">Real results our clients see when Fly's experts manage their GBP</p>
        </div>
      ...
      <div class="relative container mx-auto px-4">
        <div v-reveal class="text-center mb-16">
          <h2 class="text-4xl md:text-5xl font-bold mb-4">
            <span class="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-400">What Our Experts Do for You</span>
          </h2>
          <p class="text-gray-400 text-lg max-w-3xl mx-auto">
            Fly's Local SEO team uses Fly's platform to perform every routine Local SEO activity for your Google Business Profile, so you don't have to.
          </p>
        </div>
            ...
            <div class="relative bg-gray-900 rounded-3xl border border-gray-800 p-8 h-full flex flex-col">
              ...
              <div class="mb-4">
                <div class="flex items-baseline gap-1">
                  <span class="text-6xl font-black text-white">₹120</span>
                  <span class="text-gray-400 text-lg font-medium">/day</span>
                </div>
                <p class="text-gray-500 text-sm mt-1">per location</p>
              </div>
              <div class="bg-gray-800/50 rounded-xl p-4 mb-6">
                <div class="flex justify-between items-center">
                  <span class="text-gray-400 text-sm">Billed monthly</span>
                  <span class="text-white font-semibold text-lg">₹3,600<span class="text-gray-500 text-sm font-normal">/mo</span></span>
                </div>
              </div>
              <ul class="space-y-3 mb-8 flex-grow">
                <li v-for="(f, i) in MONTHLY_FEATURES" :key="i" class="flex items-start gap-3 text-sm text-gray-300">
                  <Check class="w-4 h-4 text-green-400 mt-0.5 flex-shrink-0" /><span>{{ f }}</span>
                </li>
              </ul>
              <a href="#contact" class="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white rounded-xl py-4 font-semibold transition-colors">
              ...
              <div class="relative bg-gray-900 rounded-3xl border border-green-500/50 p-8 h-full flex flex-col">
                ...
                <div class="mb-4">
                  <div class="flex items-baseline gap-2">
                    <span class="text-6xl font-black text-white">₹100</span>
                    <span class="text-gray-400 text-lg font-medium">/day</span>
                    <span class="text-gray-600 text-sm line-through ml-1">₹120/day</span>
                  </div>
                  <p class="text-gray-500 text-sm mt-1">per location</p>
                </div>
                <div class="bg-gray-800/50 rounded-xl p-4 mb-6 text-sm">
                  <div class="flex justify-between text-gray-400 mb-2">
                    <span>Monthly rate (12 months)</span><span class="line-through">₹43,200</span>
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-white font-semibold">Billed annually</span>
                    <span class="text-green-400 font-bold text-lg">₹36,500<span class="text-gray-500 text-sm font-normal">/yr</span></span>
                  </div>
                </div>
                <ul class="space-y-3 mb-8 flex-grow">
                  <li v-for="(f, i) in ANNUAL_FEATURES" :key="i" class="flex items-start gap-3 text-sm text-gray-300">
                    <Check class="w-4 h-4 text-green-400 mt-0.5 flex-shrink-0" /><span>{{ f }}</span>
                  </li>
                </ul>
                <a href="#contact" class="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white rounded-xl py-4 font-semibold transition-colors">
    ...
    <section class="py-24 relative bg-gradient-to-b from-gray-950 via-gray-900/50 to-gray-950">
      <div class="container mx-auto px-4">
        <div v-reveal class="text-center mb-12">
          <h2 class="text-3xl md:text-4xl font-bold mb-4 text-white">Want a Lighter Option?</h2>
          <p class="text-gray-400 text-lg max-w-2xl mx-auto">If you prefer managing your own GBP, explore Fly Lite (WhatsApp Assistant) or Fly DIY (self-service dashboard).</p>
        </div>
        <div class="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          <a href="https://fly-social.com/gmb-whatsapp-assistant" target="_blank" rel="noopener" class="block h-full group" v-reveal="0">
            <div class="relative h-full bg-gray-900/80 backdrop-blur-sm rounded-2xl border border-gray-800 hover:border-green-500/50 transition-all duration-300 p-8">
              <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-green-500 to-emerald-500 p-2.5 mb-4">
                <Smartphone class="w-full h-full text-white" />
              </div>
              <h3 class="text-xl font-bold text-white mb-2">Fly Lite, WhatsApp Assistant</h3>
              <p class="text-gray-400 text-sm mb-4">Self-serve GBP management via WhatsApp. Review alerts, AI responses, profile protection, you handle it through WhatsApp conversations.</p>
              <div class="flex items-center text-green-400 text-sm font-medium group-hover:translate-x-2 transition-transform">Learn more <ArrowRight class="w-4 h-4 ml-2" /></div>
            </div>
          </a>
          <a href="https://fly-social.com/fly-diy" target="_blank" rel="noopener" class="block h-full group" v-reveal="1">
            <div class="relative h-full bg-gray-900/80 backdrop-blur-sm rounded-2xl border border-gray-800 hover:border-cyan-500/50 transition-all duration-300 p-8">
              <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-500 p-2.5 mb-4">
                <TrendingUp class="w-full h-full text-white" />
              </div>
              <h3 class="text-xl font-bold text-white mb-2">Fly DIY, Self-Service Dashboard</h3>
              <p class="text-gray-400 text-sm mb-4">Full-featured web dashboard for hands-on businesses. Rank tracking, content publishing, multi-location management, and more.</p>
              <div class="flex items-center text-cyan-400 text-sm font-medium group-hover:translate-x-2 transition-transform">Learn More <ArrowRight class="w-4 h-4 ml-2" /></div>
            </div>
          </a>
        </div>
      </div>
    ...
    <!-- Footer -->
    <footer class="border-t border-gray-800 bg-gray-950 py-10">
      <div class="container mx-auto px-4 text-center text-sm text-gray-500 space-y-2">
        <p class="text-gray-300 font-semibold">Fly Managed Local SEO</p>
        <p>
          <a href="mailto:info@zoomlocal.in" class="hover:text-green-400 transition-colors">info@zoomlocal.in</a>
          <span class="mx-2">•</span>
          <a href="tel:+919270362196" class="hover:text-green-400 transition-colors">+91 92703 62196</a>
        </p>
        <p>© 2026 ZoomLocal Tech Private Limited, India</p>
      </div>
    </footer>
  </div>
</template>
